...
export const LEVEL_SPECS_PART2: LevelSpec[] = [
  {
    ...
    memoSections: ['Voice', 'Devices', 'Symbol & Motif', 'Clarity', 'Next-Draft Focus'],
  },
  {
    level: 5,
    tone: 'collegial',
    ...
    memoSections: ['Structure & Pacing', 'Voice', 'Clarity', 'Next-Draft Focus'],
  },
  {
    level: 6,
    tone: 'professorial',
    ...
    memoSections: ['Revision Plan', 'Beat Integrity', 'Continuity', 'Clarity', 'Next-Draft Focus'],
  },
  {
    level: 7,
    tone: 'professorial',
    ...
    memoSections: ['Logic & Stakes', 'Voice', 'Clarity', 'Next-Draft Focus'],
  },
  {
    level: 8,
    tone: 'editorial',
    ...
    memoSections: ['Signature Style', 'Device Weave', 'Structure', 'Clarity', 'Next-Draft Focus'],
  },
  {
    level: 9,
    tone: 'editorial',
    ...
    memoSections: ['Collab Readiness', 'Structure', 'Voice', 'Next-Draft Focus'],
  },
  {
    level: 10,
    tone: 'editorial',
    ...
    memoSections: ['Publication Fit', 'Line Readiness', 'Structure', 'Voice', 'Next Steps'],
  },
]
...
> = {
  ...
  worldbuilder: {
    memoHints: ['Seed setting details that carry emotional weight, not encyclopedia.'],
  },
  minimalist: {
...
export function applyWriterMods(spec: LevelSpec, writer?: WriterType): LevelSpec {
  ...
  const dimensions = spec.dimensions.map((d) => ({
    ...d,
    weight: Math.max(0, Math.min(1, d.weight + (mod.weightBoost?.[d.key] ?? 0))),
  }))
...
export function buildEvalPrompt(args: {
  ...
  const spec = LEVEL_SPECS_PART2.find((s) => s.level === args.level) ?? LEVEL_SPECS_PART2[0]
